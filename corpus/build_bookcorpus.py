...
def note_is_live(n, LIVE_ROOT):
    """Has this note survived since the snapshot that produced the manifest?

    LIVE_ROOT is passed in rather than read from module scope: the work moved behind main(), and
    a helper that silently depends on a global set by the caller is the kind of coupling that
    turns "importable" back into "runs only one way".
...
def main() -> int:
    """The work. It used to run at import, which meant `--help` read out/ocr.jsonl and a clean
    clone could not answer it — the same defect as merge_corpus, found by the same review."""
    ocr = {}
    ...
    deleted = []
    for n in man["notes"]:
        if not note_is_live(n, LIVE_ROOT):
            deleted.append(n.get("title") or n.get("path"))
            continue
        b, how = note_book(n, cat)
        if not b:
            continue
