...
> **Note (2026-09-06).** This is the design record, not the working brief. An agent pointed at
> this repository should read `AGENTS.md` at the root. Renamed from `AGENTS.md` because the
> two shared a filename and an agent told to "read AGENTS.md" reached this one first.
...
(`agents/tutor/run.sh` in v0.2 carries `stdout`, `file` and `ntfy`). The reason
